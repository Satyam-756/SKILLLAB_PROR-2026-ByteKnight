...
**Response:**  
This project transforms the Boolean Board into an 8‑channel logic analyzer. It captures signals from an Arduino Uno (or any 3.3V logic) via the Pmod header, and streams to a Python GUI over USB‑UART
...
| `Spartan-7 FPGA`               | `Processing`      | `Samples inputs at 100 MHz, checks against trigger logic.`                     |
...
| -------------------- | ------------------------ | ------------- | --------------------------- | ------------ |
| `Boolean Board`         | `High-speed 100 MHz sampling  capacity`   | `Lab Provided`     | `Day 1`                | `[Received]` |
...
| ----------- | ------------------------------------ | ------------------------------------- | ----------------------------------------- | --------------------------------------------- |
| `Archit` | `Zoomed in and out of the captured waveforms on the screen.` | `Couldn't easily tell which colored line corresponded to which Arduino pin.` | `Impressed by the speed.` | `Add clear labels (Ch 0, Ch 1, etc.) directly next to the colored lines on the Y-axis.` |
...
Our final project is a fully functional, 8-channel Embedded Logic Analyzer. The Spartan-7 FPGA successfully samples incoming digital signals from an Arduino Nano at 100 MHz, waits for a user-defined hardware trigger (like a rising edge), and captures the state of all 8 channels. Once the capture is complete, it streams the hex data over a 921600 baud USB-UART connection to a custom Python GUI on a laptop, which accurately graphs the waveforms for easy debugging.
...
Data Transfer Speed: While 921600 baud is standard, Upgrading to a faster custom USB protocol would make it feel more instantaneous.
...
Originally, we planned to stream data continuously from the FPGA to the PC in real-time and then sends it over UART to the PC to be plotted. We also added input debouncing in Verilog to handle noisy jumper wires, which wasn't in our initial scope.
...
Coding: We deepened our understanding of Verilog State Machines (FSMs). In Python, we learned how to use pyserial and optimize Matplotlib for faster plotting.
