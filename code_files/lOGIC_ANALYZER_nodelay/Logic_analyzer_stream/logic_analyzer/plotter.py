...
def plot_waveforms(
    ...
    if trigger_index is not None and 0 <= trigger_index < len(times):
        trigger_time = times[trigger_index]
        axes.axvline(trigger_time, color="#b91c1c", linestyle="--", linewidth=1.2)
        axes.text(trigger_time, 1.01, "Trigger", transform=axes.get_xaxis_transform(), color="#b91c1c")
